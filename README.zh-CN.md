...
**Draft。** Native core 当前已经通过本地格式、测试、Clippy 和 WASM 编译检查。浏览器工作台、托管 CI、发布产物和公开部署仍需形成精确到 commit 的证据，之后才能宣称达到 Alpha。

- **本地人类界面：** 已实现，尚待完成真实浏览器证据。
...
- CSV/TSV 导出保留原始单元格值。以 `=`、`+`、`-` 或 `@` 开头的值可能被电子表格软件解释为公式；使用此类软件打开不可信导出前必须检查内容。
